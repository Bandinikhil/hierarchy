...
const organizationSlice = createSlice({
  ...
  reducers: {
    ...
    //   const { departmentId, teamId, teamMember } = action.payload;
      ...
      const team = state.CEO.departments[departmentId].teams.find(t => t.id === teamId);
    ...
    moveTeamMember: (state, action) => {
        const { departmentId, currentTeamId, newTeamId, teamMemberId } = action.payload;
        ...
        const teamMemberIndex = currentTeam.teamLeader.teamMembers.findIndex(member => member.id === teamMemberId);
        ...
        const [teamMember] = currentTeam.teamLeader.teamMembers.splice(teamMemberIndex, 1);
      ...
      removeTeamMember: (state, action) => {
        ...
        const teamMemberIndex = team.teamLeader.teamMembers.findIndex(member => member.id === teamMemberId);
        if (teamMemberIndex === -1) return console.error('Team member not found in team');
    ...
    //   // Action to edit an existing team
    ...
    //       return;
    //     }
    //     const teamIndex = department.teams.findIndex(team => team.id === teamId);
    //     if (teamIndex === -1) {
    //       console.error('Team not found');
    //       return;
    //     }
    //     department.teams[teamIndex] = updatedTeam;
    //     localStorage.setItem('organizationData', JSON.stringify(state));
    //   },
    ...
    addTeam: (state, action) => {
      ...
      editTeam: (state, action) => {
        ...
        department.teams[teamIndex] = { ...department.teams[teamIndex], ...updatedTeam };
        localStorage.setItem('organizationData', JSON.stringify(state));
      },
      ...
      promoteEmployee: (state, action) => {
        ...
        const [teamMember] = team.teamLeader.teamMembers.splice(teamMemberIndex, 1);
...
export const { addTeamMember, moveTeamMember,removeTeamMember,  updateEmployeeInfo, addTeam, editTeam,promoteEmployee } = organizationSlice.actions;
