...
const AddEditTeamForm = ({ departmentId, team, onClose }) => {
  ...
  const [teamLeaderPhone, setTeamLeaderPhone] = useState(
    team ? team.teamLeader.phone : ""
  );
  const [teamLeaderEmail, setTeamLeaderEmail] = useState(
    team ? team.teamLeader.email : ""
  );
  ...
  const handleSubmit = (e) => {
    ...
    const isDuplicateName = teams.some(
      (t) => t.id !== team?.id && t.name === teamName
    );
    if (isDuplicateName) {
    ...
    // dispatching the updated data to the redux slice
    if (team) {
      dispatch(
        editTeam({ departmentId, teamId: team.id, updatedTeam: teamData })
      );
    } else {
